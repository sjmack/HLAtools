...
Indels in these alignments are numbered sequentially, following the position in the 5' direction, with decimal values appended to the 5' reference position, starting from ".1", as illustrated below.
...
Indels in *codon* alignment positions are similarly numbered from the closest codon in the N-terminal direction. However, due to a peculiarity of the R environment, which strives for unique column-header names, the last two positions of the N-terminal-ward codon will be numbered with the next highest decimal values of the insertion. Positions in codons that do not precede an insertion are numbered X, X.1 and X.2. In the example below, the *allele* and *trimmed_allele* columns have been omitted for spacing.
