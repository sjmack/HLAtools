...
The first four columns of each alignment identify the locus, allele, two-field allele name (trimmed_allele), and full allele name (allele_name) for each sequence, while the remaining columbs identify the sequence at each position, as illustrated below.
...
*alignmentFull()* is a wrapper function for *buildAlignments()* that populates the *HLAalignments* object. This function requires internet access, and can take significant time to run to build a full complement of alignments. 
