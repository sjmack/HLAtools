#BuildAlignments v1.0.0 25 January 2024
...
buildAlignments<-function(loci, source, version = "Latest"){
  ...
  #checks if loci[i] is present in the Gazeteer
  for(j in 1:length(loci)){
    if(loci[j]=="DRB") next #skip DRB
...
#  }
  ...
  version <- squashVersion(version,TRUE)
  ...
  for(i in 1:length(loci)){
    ...
    for(j in 1:length(source)){
      ...
      #if version is equal to latest, or if version is numeric and > 3310, extract version number from line 3, and skip first 7 rows and last 3 rows
      ...
      if(source[j]=="AA"){
        ...
        if(is.na(alignment_start[[loci[i]]])) {
      ...
      #creates number sequence for alignment (cDNA and gDNA tables only)
      ...
      }   else if(source[j]=="gDNA"){alignment_positions[[loci[i]]]<-as.character(unlist(strsplit(capture.output(cat(c(DNA_start[[loci[i]]]:-1,1:length(corr_table[[loci[[i]]]][3,])))), " ")))}
